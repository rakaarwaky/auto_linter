...
def test_extract_imports_from_statement():
    code = "from taxonomy.lint_result_models import LintResult\n"
    with tempfile.NamedTemporaryFile(mode="w", suffix=".py", delete=False) as f:
        f.write(code)
        path = f.name
    try:
        imports = _extract_imports(path)
        assert any("taxonomy" in m for _, m in imports)
    finally:
        os.remove(path)
...
def test_resolve_root_finds_src_parent():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        os.makedirs(src_dir)
        target = os.path.join(tmp, "src", "capabilities", "some_action.py")
...
def test_governance_adapter_no_violations():
    """A file in surfaces that only imports from capabilities should pass."""
    code = textwrap.dedent("""
        from taxonomy.lint_result_models import LintResult
    """)
    with tempfile.TemporaryDirectory() as root:
...
def test_governance_adapter_surface_imports_infrastructure():
    ...
    code = textwrap.dedent("""
        from infrastructure import RuffAdapter
    """)
    with tempfile.TemporaryDirectory() as root:
        src_dir = os.path.join(root, "src", "surfaces", "mcp")
        os.makedirs(src_dir)
...
def test_governance_adapter_capabilities_imports_infrastructure():
    """Core importing infrastructure is a CRITICAL violation."""
    code = textwrap.dedent("""
        from infrastructure.python_ast_tracer import PythonTracer
    """)
    with tempfile.TemporaryDirectory() as root:
        src_dir = os.path.join(root, "src", "capabilities", "linting")
        os.makedirs(src_dir)
        ...
        assert len(results) == 1
        assert results[0].severity == Severity.CRITICAL
        assert "capabilities" in results[0].message

def test_governance_adapter_capabilities_imports_surfaces():
    """Core importing surfaces is forbidden."""
    code = textwrap.dedent("""
        from surfaces.mcp.server import main
    """)
    with tempfile.TemporaryDirectory() as root:
        src_dir = os.path.join(root, "src", "capabilities")
        os.makedirs(src_dir)
...
def test_governance_adapter_agent_is_exempt():
    """Agent layer is exempt from all layer rules."""
    code = textwrap.dedent("""
        from infrastructure import RuffAdapter
        from surfaces.mcp.server import main
    """)
    with tempfile.TemporaryDirectory() as root:
        src_dir = os.path.join(root, "src", "agent")
        os.makedirs(src_dir)
...
def test_governance_adapter_scan_skips_pycache():
    ...
    with tempfile.TemporaryDirectory() as root:
        ...
        with open(filepath, "w") as f:
            f.write("from infrastructure import RuffAdapter\n")
...
def test_governance_adapter_multiple_violations():
    ...
    code = textwrap.dedent("""
        from infrastructure import RuffAdapter
        import infrastructure.python_tracer
    """)
    with tempfile.TemporaryDirectory() as root:
        src_dir = os.path.join(root, "src", "capabilities")
        os.makedirs(src_dir)
