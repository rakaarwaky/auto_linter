...
from src.infrastructure.adapters.js_tracer import (
    JSTracer,
    _detect_js_scope,
    _find_scope_bounds,
)
...
def test_detect_js_scope_function_declaration():
    result = _detect_js_scope("function processPayment(amount) {")
    assert result == "function processPayment"

def test_detect_js_scope_async_function():
    result = _detect_js_scope("async function fetchData() {")
    assert result == "function fetchData"

def test_detect_js_scope_class():
    result = _detect_js_scope("class PaymentService {")
    assert result == "class PaymentService"

def test_detect_js_scope_class_extends():
    result = _detect_js_scope("class StripeAdapter extends BaseAdapter {")
    assert result == "class StripeAdapter"

def test_detect_js_scope_arrow_function():
    result = _detect_js_scope("const calculateTotal = (items) => {")
    assert result == "function calculateTotal"

def test_detect_js_scope_no_match_control_flow():
    # 'if' and 'for' should not be detected as scoped functions
    result_if = _detect_js_scope("    if (condition) {")
    result_for = _detect_js_scope("    for (let i = 0; i < n; i++) {")
    assert result_if is None or "if" not in (result_if or "")
    assert result_for is None or "for" not in (result_for or "")

def test_detect_js_scope_detects_without_brace():
    # _detect_js_scope detects the name regardless of braces on the line.
    # The brace guard is handled by the caller (show_enclosing_scope).
    result = _detect_js_scope("function foo()")
    assert result == "function foo"

def test_find_scope_bounds_finds_bounds():
    lines = [
        "function test() {\n",   # 1
        "  const x = 1;\n",      # 2
        "  return x;\n",         # 3
    ...
    start, end = _find_scope_bounds(lines, scope_line=1)
    assert start == 1
    assert end == 4

def test_find_scope_bounds_none_scope_line():
    start, end = _find_scope_bounds([], scope_line=None)
